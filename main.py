...
UPDATED: Now supports CID-based image attachments for templates 2 and 3
"""
...
from template2 import generate_template2_html_with_cid
from template3 import generate_template3_html_with_cid
...
def generate_email_html(analysis: Dict[str, Any], location_name: str, today_str: str,
                       template_no: int) -> Tuple[str, Optional[Dict[str, bytes]]]:
    """
    Generate email HTML using selected template
    
    Returns:
        Tuple[str, Optional[Dict[str, bytes]]]: (HTML content, optional chart images dict)
        - For template 1: (html, None)
        - For templates 2 & 3: (html, {cid: image_bytes})
    """
    if template_no == 3:
        html_content, chart_images = generate_template3_html_with_cid(analysis, location_name, today_str)
        return html_content, chart_images
    elif template_no == 2:
        html_content, chart_images = generate_template2_html_with_cid(analysis, location_name, today_str)
        return html_content, chart_images
    else:
        html_content = generate_template1_html(analysis, location_name, today_str)
...
def send_email_with_attachments_ses(from_email: str, to_email: str, subject: str, 
                                    ...
                                    chart_images: Optional[Dict[str, bytes]] = None) -> None:
    """
    ...
    Args:
        ...
        attachments: List of CSV attachments
        chart_images: Optional dict of {cid: image_bytes} for inline images
    """
    
    # Use 'related' multipart type if we have inline images, otherwise 'mixed'
    msg = MIMEMultipart('related' if chart_images else 'mixed')
    msg['Subject'] = subject
    msg['From'] = from_email
    msg['To'] = to_email
    ...
    # Attach inline images if present (for templates 2 & 3)
    if chart_images:
...
Generated on: {datetime.now().strftime("%d/%m/%Y at %H:%M")}"""
                    ...
                    send_email_with_attachments_ses(
                        ...
                        no_data_text,
                        [],
                        None  # No chart images for no-data emails
                    )
            ...
            try:
                ...
                # Generate HTML with owner's template (returns HTML and optional chart images)
                html_content, chart_images = generate_email_html(analysis, location_name, today_str, owner_template_no)
                
                # Log chart images info
                if chart_images:
                    logger.info(f"Template {owner_template_no} generated {len(chart_images)} inline images")
...
Template Used: {owner_template_no} {'(Modern Business Intelligence)' if owner_template_no == 3 else '(Modern Analytics)' if owner_template_no == 2 else '(Classic Business)'}
Generated on: {datetime.now().strftime("%d/%m/%Y at %H:%M")}"""
                ...
                # Send email using SES API with chart images if present
                send_email_with_attachments_ses(
                    from_email,
                    "a6hinandh@gmail.com",
                    f"{'Business Intelligence Report' if owner_template_no == 3 else 'Business Analytics Report' if owner_template_no == 2 else 'Daily Report'} - {today_str} - {location_name}",
                    html_content,
                    text_content,
                    attachments,
                    chart_images  # Pass chart images for templates 2 & 3
                )
                ...
                chart_info = f", {len(chart_images)} inline images" if chart_images else ""
                logger.info(f"Email sent to {owner['email']} ({len(owner_today_logs)} approved records, ₹{analysis['totalRevenue']:,}, {len(attachments)} CSV attachments{chart_info}, Template {owner_template_no})")
                ...
                email_results.append({
                    ...
                    'inlineImages': len(chart_images) if chart_images else 0,
                    'templateUsed': owner_template_no,
...
Generated on: {datetime.now().strftime("%d/%m/%Y at %H:%M")}"""
            ...
            send_email_with_attachments_ses(
                ...
                summary_text,
                [],
                None  # No chart images for summary report
            )
        ...
        return jsonify({
            ...
            'deliveryMethod': 'AWS SES API with CID inline images',
            'templatesSupported': {
                '1': 'Classic Business (no inline images)',
                '2': 'Modern Analytics (with CID charts)',
                '3': 'Business Intelligence (with CID charts)'
            },
            'results': email_results
        }), 200
...
def health():
    ...
    return jsonify({
        ...
        'features': 'CID inline images for templates 2 & 3'
    }), 200
