...
async def fetch_today_filtered_logs(location_id: Optional[str] = None) -> List[Dict[str, Any]]:
...
async def send_reports():
    ...
    try:
        ...
        for owner in (owners or []):
            ...
            # Fetch logs
            try:
                owner_today_logs = await fetch_today_filtered_logs(current_location)
            except Exception as e:
...
{'🧪 Test Mode: Sent to ' + test_email if test_email else ''}"""
        ...
        total_approved_logs = await fetch_today_filtered_logs()
